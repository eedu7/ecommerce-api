...
async def get_by_user_id(
    cart_crud: CartCRUD = Depends(get_cart_crud), user: User = Depends(get_current_user)
):
...
async def create_cart(
    user: User = Depends(get_current_user), cart_crud: CartCRUD = Depends(get_cart_crud)
):
